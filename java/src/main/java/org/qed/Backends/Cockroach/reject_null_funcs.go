...
func deriveGroupByRejectNullCols(
	...
	for i := range aggs {
		...
		// Get column ID of aggregate's Variable operator input. If the
		// input is not a simple column reference (e.g. it is a complex
		// expression that was merged in by AggregateProjectMerge), null
		// rejection is not possible.
		v, ok := agg.Child(0).(*memo.VariableExpr)
		if !ok {
			return opt.ColSet{}
		}
		inColID := v.Col
