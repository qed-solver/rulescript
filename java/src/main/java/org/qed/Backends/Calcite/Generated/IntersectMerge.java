...
public class IntersectMerge extends RelRule<IntersectMerge.Config> {
	...
	public interface Config extends EmptyConfig {
		...
		default RelRule.OperandTransform operandSupplier() {
			return s_4 -> s_4.operand(LogicalIntersect.class).predicate(intersect -> !intersect.all).inputs(s_2 -> s_2.operand(LogicalIntersect.class).predicate(intersect -> !intersect.all).inputs(s_0 -> s_0.operand(RelNode.class).anyInputs(), s_1 -> s_1.operand(RelNode.class).anyInputs()), s_3 -> s_3.operand(RelNode.class).anyInputs());
		}
